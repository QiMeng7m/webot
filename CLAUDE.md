...
5. **消息发送** — 需要微信窗口可见
6. **修仙玩法命令** — 需要微信客户端 + 真实群聊：@bot 发 `修炼` / `我的` / `排行榜` / `突破`，确认回复正确且角色数据落库（`game_characters` 表）
7. **修仙被动收益与机缘** — 群里正常聊天确认修为增长且有 30s 冷却；把 `GAME_ENCOUNTER_INTERVAL_MIN` 临时调成 1，确认机缘播报出现且只有第一个回复「抢机缘」的人拿到灵石
